...
set start_fmt = `date +%s`
set start = `date`
python ../../../src/brat2ner_bulkdoc_multiword.py --in ${train_name}.list --out ${train_name}.tsv > ${train_name}.log
set end_fmt = `date +%s`
set end = `date`
@ run = $end_fmt - $start_fmt
...
echo "gazette=MERA_targets_minerals-2017-05_elements.gaz.txt" >> lpsc_${train_name}.prop
