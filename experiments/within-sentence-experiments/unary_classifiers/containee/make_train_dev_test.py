...
def make_instances( tokenizer, ann_files, text_files, corenlp_files, outdir, max_len = 512, is_training = False, use_sys_ners = False):
    """
    This function extract training, dev and testing component instances from files

    Args:
        tokenizer: 
            a bert tokenizer 
        ann_files: 
            a list of ann files ( used to assign gold labels (binary label for whether the instance is a Containee instance) to component instances)
        text_files:
            a list of text files where component instances would be extracted 
        corenlp_files:
