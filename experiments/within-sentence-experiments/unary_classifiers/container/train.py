...
def train(model, train_dataloader, val_dataloader, val_gold_ins, args):
    ...
    """ 
    This function trains a Container model 

    Args:
        model:
            model to train 
        train_dataloder:
            a dataloader that contains training Container instances 
        val_dataloder:
            a dataloader that cotnains extracted target instances from development data 
        val_gold_ins:
            a list of gold container instances from DEV data. This is used to evaluate the model over the  development data (val_dataloader doesn't have gold labels. It only provides target instances for the model to make predictions for).
        args:
