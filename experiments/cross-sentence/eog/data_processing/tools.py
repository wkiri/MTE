...
def adjust_offsets(old_sents, new_sents, old_entities, show=False, is_mars = False, return_error_counts  = 0 ):
    """
    ...
    for s in new_sents:
        ...
        cur += len(s) + 1
    if is_mars: # we dont want to join the sentences by space since this would mess up offsets in texts where sentences do not always start with empty space
    ...
    ent_sequences = []
    # checking if the offset matches the text 
            ...
            if condition:
                ...
                for s_no, sr in enumerate(new_sent_range):
                    if set(np.arange(term[0], term[1])).issubset(set(np.arange(sr[0], sr[1]))):
                        sent_no += [s_no]
                try: #added by yuan. sometimes sent_no is empty because of unfound entities (parsing error)
                    assert (len(sent_no) == 1), '{} ({}, {}) -- {} -- {} <> {}'.format(sent_no, term[0], term[1],
                                                                                   new_sent_range,
                                                                                   newtext[term[0]:term[1]], term[3])
                except:
                    error_counts += 1 
                    print(f'Unfound error in {term[4]}')
                    print('{} ({}, {}) -- {} -- {} <> {}'.format(sent_no, term[0], term[1],new_sent_range,newtext[term[0]:term[1]], term[3]))
                    continue
    ...
    if return_error_counts:
        return new_entities, error_counts
    else:
        return new_entities
