...
then
  ...
  python3 predict.py -modelfile "$modelSaveDir/trained_model.ckpt" -test_dir $testdir -analyze_dev 0 -outdir $modelSaveDir/dev -threshold $pred_threshold
fi 
...
python3 predict.py -modelfile "$modelSaveDir/trained_model.ckpt" -test_dir $testdir -analyze_dev 0 -outdir $modelSaveDir/test -threshold $pred_threshold
fi
...
then
mkdir $modelSaveDir/dev
python3 predict.py  -modelfile "$modelSaveDir/trained_model.ckpt" -test_dir ins/dev -predict_with_extracted_gold_entities 1 -analyze_dev 0 -outdir $modelSaveDir/dev -threshold $pred_threshold
fi 

if [ "$eval_container_test" = "1" ]
then
mkdir $modelSaveDir/test
python3 predict.py  -modelfile "$modelSaveDir/trained_model.ckpt" -test_dir ins/test -predict_with_extracted_gold_entities 1 -analyze_dev 0 -outdir $modelSaveDir/test -threshold $pred_threshold
fi
