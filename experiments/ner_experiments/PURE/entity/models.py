...
class EntityModel():
    ...
    def _get_input_tensors(self, tokens, spans, spans_ner_label):
        ...
        # added by yuan:  limit each sentence to be less than 512 . each span is startidx, endidx, span width

        # bert_spans = [[start2idx[span[0]], end2idx[span[1]], span[2]] for span in spans]
        bert_spans = [[start2idx[span[0]], end2idx[span[1]], span[2]] for span in spans if start2idx[span[0]] < 512 and end2idx[span[1]] < 512 ]
        spans_ner_label = [spans_ner_label[i] for i in range(len(spans_ner_label)) if start2idx[spans[i][0]] < 512 and end2idx[spans[i][1]] < 512]
        ...
        if training:
            ...
            for i, sample in enumerate(samples_list):
                ...
                for j in range(len(sample['spans'])):
                    # added yuan
                    if not (i < len(predicted_label) and j < len(predicted_label[i])):
                        continue
