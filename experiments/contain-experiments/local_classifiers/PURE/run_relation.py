...
def print_pred_json(eval_data, eval_examples, preds, logits, id2label, output_file):
    rels = dict()
    #ADDED
    for ex, pred,logit in zip(eval_examples, preds, logits):
        ...
        # ADDED
        if pred != 0:
            rels[doc_sent].append([sub[0], sub[1], obj[0], obj[1],logit, id2label[pred]])
        else:
            rels[doc_sent].append([sub[0], sub[1], obj[0], obj[1],logit, 'O'])
...
def main(args):
    ...
    if args.eval_test:
        test_dataset, test_examples, test_nrel = generate_relation_data(args.test_file, use_gold=True, context_window=args.context_window)
    ...
    if args.do_eval:
        ...
        print_pred_json(eval_dataset, eval_examples, preds, logits.tolist(), id2label, os.path.join(args.output_dir, args.prediction_file))
