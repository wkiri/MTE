...
def generate_example_id(venue, year, fnbase, char_start_end1, char_start_end2, tok_idx1, tok_idx2, sentid, org_text1, org_text2, entity_ner_label1, entity_ner_label2, org_before_text1, org_before_text2):
    ...
    return json.dumps([venue, year,fnbase,char_start_end1, char_start_end2, tok_idx1, tok_idx2, sentid, org_text1, org_text2, entity_ner_label1, entity_ner_label2, org_before_text1, org_before_text2])
...
def build_jsre_examples(mode, relation_to_assign,textfiles, out_path,  corenlp_url, use_gold_entities):
    ...
    for numfile, textfile in enumerate(textfiles):
        stdout.write("\rprocessing %s/%s documents. Generated word-word examples, including %s positive and %s negative" % (str(numfile), str(len(textfiles)), count_examples[1], count_examples[0]))
        stdout.flush()
        ...
        try: # works for lpsc15 and 16 
            ...
            venue=textfile.split("/")[-2]

        except:
            year, fnbase = re.findall(r"(\d+)_(\d+)", in_fn.split("/")[-1])[0]
            venue=textfile.split("/")[-2]

        ann_fn = textfile.split(".txt")[0] + ".ann"

        out_fn = '%s-%s-%s-%s.examples' % (venue, year, fnbase, relation_to_assign.lower())
        # print 'Reading in %s and %s' % (in_fn, ann_fn)
        accept_entities = set(accept_ner_labels)
        text, tree = parse(in_fn, ann_fn, accept_entities)
        ...
        for entity_text1, entity_text2, _ in entity_entity_relation:
            gold_entity_entity_relation.append((venue, year, fnbase, canonical_target_name(entity_text1), canonical_elemin_name(entity_text2)))
                ...
                for i in range(len(tokens)):
                    ...
                    for j in range(len(tokens)):
                        ...
                        if keep_example(target, active, annotid2position_ner,use_gold_entities):
                            ...
                            id = generate_example_id(venue, year, fnbase, char_start_end1, char_start_end2, tok_idx1, tok_idx2, sentid, org_text1, org_text2, entity_ner_label1, entity_ner_label2, org_before_text1, org_before_text2)
    ...
    with io.open(out_eval_fn, 'w', 
                 encoding='utf8') as outf:
        for venue, year, fnbase, std_entity_text1, std_entity_text2 in sorted(set(gold_entity_entity_relation), key = lambda x: x[1]):  
            to_write = "%s\t%s\t%s\t%s\t%s\n"%(venue, year, fnbase, std_entity_text1, std_entity_text2)
    ...
    print("In eval set, there are %s unique tuples " % (str(len(set(gold_entity_entity_relation)))))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_mpfphx', choices = [0,1], type = int, help = "whether or not to add mpf and phx into test set", required= True)
    parser.add_argument('--use_mpfphx_only', choices = [0,1], type = int, help = "whether or not to use only mpf and phx as test set", default = 0)
    ...
    textfiles15 = sorted(glob.glob(join(datapath, "lpsc15-C-raymond-sol1159-v3-utf8/*.txt")))
    textfiles16 = glob.glob(join(datapath, "lpsc16-C-raymond-sol1159-utf8/*.txt"))
    trainfiles = textfiles15[:42]
    valfiles = textfiles15[42:]
    testfiles = textfiles16
    ...
    def keep_file(annfile):
        with open(annfile, "r") as f:
            text = f.read().strip()
        if not re.search("Target", text) or not (re.search("Element", text) or re.search("Mineral", text)):
            return False 
        return True

    if args.add_mpfphx:
        for file in glob.glob(join(datapath, "mpf-reviewed+properties-v2/*_*.txt")) + glob.glob(join(datapath, "phx-reviewed+properties/*_*.txt")): # do a filtering here to filter out ann files that have no targets or elements/minerals
            annfile = file.split(".txt")[0] + ".ann"
            if keep_file(annfile):
                testfiles.append(file)
    
    if args.use_mpfphx_only:
        testfiles = []
        for file in glob.glob(join(datapath, "mpf-reviewed+properties-v2/*_*.txt")) + glob.glob(join(datapath, "phx-reviewed+properties/*_*.txt")): # do a filtering here to filter out files without any contains relatons in ann file 
            annfile = file.split(".txt")[0] + ".ann"
            if keep_file(annfile):
                testfiles.append(file)

    
    outdir = join("jsre-examples", mode)
    if exists(outdir):
        rmtree(outdir)
    ...
    use_gold_entities = 1
    out_path = join(outdir, "train")
    build_jsre_examples(mode, relation_to_assign, trainfiles, out_path,  corenlp_url, use_gold_entities)
    # # ==== make val examples
    use_gold_entities = 1
    out_path = join(outdir, "val")
    build_jsre_examples(mode, relation_to_assign, valfiles, out_path,  corenlp_url, use_gold_entities)
    # # # ==== make test examples
    ...
    # use_gold_entities = 0 # use system entities to predict relations
    # out_path = join(outdir, "test-sysentity")
    # build_jsre_examples(mode, relation_to_assign,testfiles, out_path,  corenlp_url, use_gold_entities)
