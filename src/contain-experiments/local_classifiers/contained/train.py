...
def eval_and_save(model, val_dataloader, val_gold_ins,  best_f1, args, label_to_eval = "Contains", tuple_level = False, save_prediction = False): 
    ...
    if should_save and args.save_model:
        ...
        if args.analyze_dev:
            ...
            analayze(pred_instances, val_gold_ins, dev_analysis_outdir)

    if save_prediction:
            with open(join(args.model_save_dir, f"{args.mode}.spans.pred"), "wb") as f:
...
def train(model, train_dataloader, val_dataloader, val_gold_ins, args):
    ...
    loss_funct = torch.nn.CrossEntropyLoss(weight = weight.to(device))
...
def downsample(ins, times):
    pos = [i for i in ins if i.relation_label == "Contains"]
    neg = [i for i in ins if i.relation_label == "O"]

    neg = neg[:(len(pos) * times)]

    new_ins = pos+neg
    random.Random(100).shuffle(new_ins)
    return new_ins
...
if __name__ == "__main__":
    ...
    with open(join(args.train_dir, f"{mode}.extracted_gold_spanins.pkl"), "rb") as f:
        ...
        # print("down sampling ... ")
        # train_ins = downsample(train_ins, 5)
        

    with open(join(args.val_dir, f"{mode}.extracted_gold_spanins.pkl"), "rb") as f:
        val_ins = pickle.load(f)


    with open(join(args.val_dir, f"{mode}.annotated_gold_spanins.pkl"), "rb") as f:
        val_gold_ins = pickle.load(f)

    if args.predict_with_extracted_gold_entities:        
        print("using gold entities for evaluation")
        test_file = join(args.test_dir, f"{mode}.extracted_gold_spanins.pkl")

    else:
        print("using system entities for evaluation")
        test_file = join(args.test_dir, f"{mode}.extracted_system_spanins.pkl")
    ...
    with open(join(args.test_dir, f"{mode}.annotated_gold_spanins.pkl"), "rb") as f:
...
#     # """ ================ train ================ """
    ...
    model = train(model, train_dataloader, val_dataloader, val_gold_ins, args)

    print("eval over test ... ")
    args.model_save_dir = join(args.model_save_dir, "test")
    args.analyze_dev = 1
    print("Instance Level Evaluation: ")
    eval_and_save(model, test_dataloader, test_gold_ins, None, args, tuple_level = False, save_prediction = True)
    print("Tuple Level Evaluation:")
    eval_and_save(model, test_dataloader, test_gold_ins, None, args, tuple_level = True)
