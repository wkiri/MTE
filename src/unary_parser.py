...
import os, json, torch, logging, numpy as np, argparse, re, random, string
...
from name_utils import canonical_component_name, targettab
...
def collate(batch):
    ...
    item["labels"] = torch.LongTensor([label2ind[ins.relation_label] for ins in batch]) if batch[
                                                                                               0].relation_label is not None else torch.LongTensor(
        [-1] * batch_size)
...
class Model(torch.nn.Module):
    ...
    def forward(self, item):
        ...
        start_embeddings1 = torch.gather(last_hidden_states, 1, start_indices1).view(batch_size,
                                                                                     -1)  # shape (batch, dimension)

        end_indices1 = ends1.view(batch_size, -1).repeat(1, dimension).unsqueeze(1)  # (batch, 1, dimension)
        end_embeddings1 = torch.gather(last_hidden_states, 1, end_indices1).view(batch_size,
                                                                                 -1)  # shape (batch, dimension)
...
class UnaryParser(CoreNLPParser):
    ...
    def __init__(self, corenlp_server_url, ner_model_file, gazette_file, containee_model_file, container_model_file, gpu_id=0):
        """
        Args:
        ...
        super(UnaryParser, self).__init__(corenlp_server_url, ner_model_file, gazette_file, 'unary_parser')
    ...
    def add_entities(self, queue, e):
        ...
        if last_e['label'] == e['label']:
            ...
            if len(queue) > 1 and queue[-1]['text'] in ["_", "-"] and queue[-2]['label'] == e[
                'label']:  # words that are splitted by hyphen or underscores
    ...
    def parse(self, text, batch_size=10, entity_linking_method='closest_container_closest_containee'):
        ...
        if entity_linking_method not in entity_linking_methods:
            raise NameError("Unrecognized entity linking method: %s. You need to choose from [%s] !" % (
            entity_linking_method, ', '.join(entity_linking_methods)))

        corenlp_dict = super(UnaryParser, self).parse(text)

        entities = [e for e in self.extract_entities(corenlp_dict, use_component=True) if
                    e['label'] in ['Target', 'Component']]
        ...
        for sentid, sent_entities in sentid2entities.items():
            ...
            for e in sent_entities:
                ...
                if span.span_id not in seen_spanids:
                    ...
                    exceed = span.insert_type_markers(tokenizer,
                                                      max_len=512)  # insert entity markers around the entity in its sentence, convert the sentence to token ids and check if the insertion makes the number of token ids more than 512
        ...
        logger.info(
            'Collected %d Targets and %d Components that co-occur with Components/Targets in the same sentence for relation inference.' % (
            len(target_instances), len(component_instances)))
        logger.info(
            '%d of them exceed 512 tokens after inserting entity markers in the sentences.' % (exceed_len_cases))
...
def process(in_file, in_list, out_file, log_file, tika_server_url, ads_url, ads_token, corenlp_server_url, ner_model,
            gazette_file, containee_model_file, container_model_file, entity_linking_method, gpu_id, batch_size):
    ...
    unary_parser = UnaryParser(corenlp_server_url, ner_model, gazette_file, containee_model_file, container_model_file, gpu_id=gpu_id)
    ...
    for f in tqdm(files):
        try:
            ...
            unary_dict = unary_parser.parse(ads_dict['content'], batch_size=batch_size,
            ...
            ads_dict['metadata']['mte_parser'].append(
                unary_parser.__class__.__name__)
